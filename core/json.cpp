...
json_value_copy(const JSON_Value &self, memory::Allocator *allocator)
{
	switch (self.kind)
	{
		...
		default:
		{
			core::assert(false, "[JSON]: Invalid JSON_VALUE_KIND.");
			return JSON_Value{};
		}
	}
}
...
{
	core::assert(self.kind == JSON_VALUE_KIND_OBJECT, "[JSON]: Expected JSON_VALUE_KIND_OBJECT.");
...
{
	core::assert(self.kind == JSON_VALUE_KIND_BOOL, "[JSON]: Expected JSON_VALUE_KIND_BOOL.");
	return self.as_bool;
}
...
{
	core::assert(self.kind == JSON_VALUE_KIND_NUMBER, "[JSON]: Expected JSON_VALUE_KIND_NUMBER.");
...
{
	core::assert(self.kind == JSON_VALUE_KIND_STRING, "[JSON]: Expected JSON_VALUE_KIND_STRING.");
...
{
	core::assert(self.kind == JSON_VALUE_KIND_ARRAY, "[JSON]: Expected JSON_VALUE_KIND_ARRAY.");
