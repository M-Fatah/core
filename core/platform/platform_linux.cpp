...
platform_allocator_deinit(Platform_Allocator *self)
{
	...
	core::assert(result == 0, "[PLATFORM]: Failed to free virtual memory.");
}
...
platform_window_init(u32 width, u32 height, const char *title)
{
	...
	{
		core::assert(false, "[PLATFORM]: Failed to connect to X server via XCB.");
		return {};
	}
	...
	if (stream_result <= 0)
	{
		core::assert(false, "[PLATFORM]: An error occurred when flusing the stream.");
		return {};
	}
